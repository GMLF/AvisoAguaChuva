...
export default function CardTableDonation({ color }) {
  ...
  const handleStatusChange =async (row) => {
    const res=await axios.put("http://localhost:3000/alteraStatusDoacao",row)
    setTeste(1)
    setTeste(0)
    return res
  };
...
<td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      {row.status?<div>Disponível para doação</div>:<div>Doado</div>}
                  </td>
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
...
CardTableDonation.defaultProps = {
  color: "light",
};

CardTableDonation.propTypes = {
